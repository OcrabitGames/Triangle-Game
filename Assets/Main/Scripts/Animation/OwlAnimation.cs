...
public class OwlAnimation : MonoBehaviour {
    ...
    private NPCMovement _owlMovement;
    ...
    void Start() {
        ...
        _owlMovement = GetComponent<NPCMovement>();
    ...
    void Update() {
        // Animate running
        if (_owlMovement.xVel != 0f || _owlMovement.zVel != 0f) {
        ...
        if (_owlMovement.xVel < 0f) {
            transform.localScale = new Vector3(Mathf.Abs(transform.localScale.x), transform.localScale.y, transform.localScale.z);
        }
        else if (_owlMovement.xVel > 0f) {
            transform.localScale = new Vector3(-Mathf.Abs(transform.localScale.x), transform.localScale.y, transform.localScale.z);
        }
        ...
        if (dustTimer <= 0f && (_owlMovement.xVel != 0f || _owlMovement.zVel != 0f)) {
