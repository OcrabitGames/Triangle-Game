...
public class FoxAnimation : MonoBehaviour {
    ...
    void Start() {
        ...
        if (type == "Purple Fox") {
            runCycle = _textureManager.purpleFoxRunCycle;
            blinkCycle = _textureManager.foxBlinkCycle;
        }
        else if (type == "Pink Fox") {
            runCycle = _textureManager.pinkFoxRunCycle;
            blinkCycle = _textureManager.foxBlinkCycle;
        }
        else if (type == "Blue Fox") {
            runCycle = _textureManager.blueFoxRunCycle;
            blinkCycle = _textureManager.foxBlinkCycle;
        }
    ...
    void Update() {
        // Animate running
        if (_foxMovement.xVel != 0f || _foxMovement.zVel != 0f) {
        ...
        if (_foxMovement.xVel < 0f) {
            transform.localScale = new Vector3(Mathf.Abs(transform.localScale.x), transform.localScale.y, transform.localScale.z);
        }
        else if (_foxMovement.xVel > 0f) {
            transform.localScale = new Vector3(-Mathf.Abs(transform.localScale.x), transform.localScale.y, transform.localScale.z);
        }
